...
dir=exp/$mic/nnet3/clstm${speed_perturb:+_sp}${affix:+_$affix}${use_delay:+_ld$label_delay}
...
if [ $stage -le 8 ]; then
  ...
  steps/nnet3/clstm/train.sh $ivector_opts \
    --stage $train_stage \
    ...
    --cmd "$decode_cmd" \
    --num-clstm-layers $num_lstm_layers \
    --cell-dim $cell_dim \
