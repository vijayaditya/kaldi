...
if [ "$use_ihm_ali" == "true" ]; then
  ...
  # check if all the necessary directories are present
  if [ ! -d exp/ihm/tri4a ]; then
    echo "Run run_ihm.sh till tri4a stage before running this script" && exit 1;
  fi
  ...
  mic=${mic}_cleanali
  ali_dir=exp/ihm/tri4a_train_parallel_sp_ali
else
  if [ ! -d exp/$mic/tri4a ]; then
    echo "Run $mic GMM-HMMs till tri4a stage before running this script" && exit 1;
  fi
  # prepare the perturbed data directory and generate alignments
  local/nnet3/prepare_perturbed_alignments.sh --mic $mic
  
  ali_dir=exp/$mic/tri4a_train_sp_ali
fi
