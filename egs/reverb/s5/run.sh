...
if [ ! -e exp/mono0a/final.mdl ]; then
    ...
    --boost-silence 1.25 --nj $nj_train \
...
if $do_tri2a; then
  ...
  for recog in tri2a tri2a_mc; do
    graph_dir=exp/$recog/graph_bg_5k
    $mkgraph_cmd $graph_dir/mkgraph.log  \
      utils/mkgraph.sh data/lang_test_bg_5k exp/tri2a $graph_dir
  done

  # decode REVERB dt using tri2a, clean
  for dataset in data/REVERB_dt/SimData_dt* data/REVERB_Real_dt/RealData_dt*; do
    steps/decode.sh --cmd "$decode_cmd" --nj $nj_bg \
  ...
  steps/get_fmllr_basis.sh --cmd "$decode_cmd" \
    --per-utt true data/REVERB_tr_cut/SimData_tr_for_1ch_A data/lang exp/tri2a_mc || exit 1;

  # Recognition using fMLLR adaptation (per-utterance processing).
  for dataset in data/REVERB_dt/SimData_dt* data/REVERB_Real_dt/RealData_dt*; do
    steps/decode_basis_fmllr.sh --cmd "$decode_cmd" --nj $nj_bg \
...
for base_recog in tri2b_mc; do
    ...
    if [ ! -e exp/$denlats_dir/.done.1 ]; then
        ...
        steps/make_denlats.sh --cmd "$decode_cmd" \
          --sub-split $subsplit --nj $nj_train \
        ...
        for dataset in data/REVERB_dt/SimData_dt* \
            ...
            if $do_adapt; then
                if [ ! -e exp/$recog/fmllr.basis ]; then
                    ...
                    steps/get_fmllr_basis.sh --cmd "$decode_cmd" \
                      --per-utt true data/$tr_dataset data/lang exp/$recog || exit 1;
                fi
