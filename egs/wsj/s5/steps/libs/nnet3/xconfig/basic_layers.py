...
class XconfigLayerBase(object):
    ...
    def __init__(self, first_token, key_to_value, all_layers):
        ...
        if not xutils.IsValidLineName(self.name):
            raise xparser_error("Invalid value: name={0}".format(key_to_value['name']), self.str())
    ...
    def SetConfigs(self, key_to_value, all_layers):
        ...
        for key,value in key_to_value.items():
            if key != 'name':
                if not key in self.config:
                    raise xparser_error("Configuration value {0}={1} was not expected in "
                                    "layer of type {2}".format(key, value, self.layer_type), self.str())
                self.config[key] = xutils.ConvertValueToType(key, type(self.config[key]), value)
        ...
        for key in self.GetInputDescriptorNames():
            ...
            if desc_norm_str != desc_norm_str2:
                raise xparser_error("Likely code error: '{0}' != '{1}'".format(
                        desc_norm_str, desc_norm_str2), self.str())
    ...
    def ConvertToDescriptor(self, descriptor_string, all_layers):
        prev_names = xutils.GetPrevNames(all_layers, self)
        tokens = xutils.TokenizeDescriptor(descriptor_string, prev_names)
        pos = 0
        (descriptor, pos) = xutils.ParseNewDescriptor(tokens, pos, prev_names)
    ...
    def GetDimForDescriptor(self, descriptor, all_layers):
        layer_to_dim_func = lambda name: xutils.GetDimFromLayerName(all_layers, self, name)
    ...
    def GetStringForDescriptor(self, descriptor, all_layers):
        layer_to_string_func = lambda name: xutils.GetStringFromLayerName(all_layers, self, name)
...
class XconfigInputLayer(XconfigLayerBase):
    ...
    def CheckConfigs(self):
        if self.config['dim'] <= 0:
            raise xparser_error("Dimension of input-layer '{0}' should be positive.".format(self.name), self.str())
...
class XconfigOutputLayer(XconfigLayerBase):
    ...
    def SetDefaultConfigs(self):
        ...
        self.config = { 'input':'[-1]',
                        ...
                        'presoftmax-scale-file':'' # used in DNN (not RNN) training using frame-level objfns
                        }

    def CheckConfigs(self):
        if self.config['dim'] <= -1:
            raise xparser_error("In output-layer, dim has invalid value {0}".format(self.config['dim']), self.str())
        if self.config['objective-type'] != 'linear' and self.config['objective_type'] != 'quadratic':
            raise xparser_error("In output-layer, objective-type has invalid value {0}".format(
                    self.config['objective-type']), self.str())
        if self.config['learning-rate-factor'] <= 0.0:
            raise xparser_error("In output-layer, learning-rate-factor has invalid value {0}".format(
                    self.config['learning-rate-factor']), self.str())
    ...
    def OutputName(self, auxiliary_outputs = None):
        ...
        raise xparser_error("Outputs of output-layer may not be used by other layers", self.str())
...
class XconfigBasicLayer(XconfigLayerBase):
    ...
    def CheckConfigs(self):
        ...
        if self.config['norm-target-rms'] < 0.0:
            raise xparser_error("norm-target-rms has invalid value {0}".format(self.config['norm-target-rms']), self.str())
        ...
        for config_name in [ 'ref', 'final' ]:
            ...
            for nonlinearity in nonlinearities:
                if nonlinearity == 'relu':
                    ...
                    raise xparser_error("Unknown nonlinearity type: {0}".format(nonlinearity), self.str())
                ans.append((config_name, line))
...
class XconfigAffineLayer(XconfigLayerBase):
    ...
    def SetDefaultConfigs(self):
        ...
        self.config = { 'input':'[-1]',
                        'param-stddev' : -1, # this has to be initialized to 1/sqrt(input_dim)
                        'bias-stddev' : 1.0,
                        'bias-mean' : 0.0,
                        'max-change' : Nones.float,
                        'dim':Nones.int}
    ...
    def GetFullConfig(self):
        ...
        option_string=''
        for key in ['param-stddev', 'bias-stddev']:
            if self.config[key] is not Nones.float:
                option_string += ' {0}={1}'.format(key, self.config[key])

        conf_lines = []
        # write the 'real' component to final.config
        conf_lines.append('component name={0} type=NaturalGradientAffineComponent '
                          'input-dim={i} output-dim={o} {opts}'.format(i = input_dim,
                                                                       o = output_dim,
                                                                       opts = option_string))
        # the component-node gets written to final.config and ref.config.
