...
It has been separated from the utils module to
    1. remove circular dependency between utils and layers
    2. isolate the code which involves addition of new layers from the utils module.
"""
...
config_to_layer = {
        ...
        'relu-norm-layer' : xlayers.XconfigBasicLayer,
...
def ParsedLineToXconfigLayer(first_token, key_to_value, prev_names):
    try:
        config_to_layer[first_token](first_token, key_to_value, prev_names)
    except KeyError:
        raise xparser_error("Error parsing xconfig line (no such layer type): " +
                        first_token + ' ' +
...
def ConfigLineToObject(config_line, prev_names = None):
    (first_token, key_to_value) = xutils.ParseConfigLine(config_line)
