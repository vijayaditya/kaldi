...
# configs for 'chain'
affix=
stage=14 # assuming you already ran the xent systems
train_stage=1383
get_egs_stage=-10
dir=exp/chain/cwrnn_asp2
decode_iter=

# training options
num_epochs=4
remove_egs=false
common_egs_dir=exp/chain/cwrnn_asp2/egs
num_data_reps=3
...
if [ $stage -le 14 ]; then
  # start from stage 0, if you didn't run any other nnet3 recipes
  local/chain/prep_test_aspire.sh --stage 4 --decode-num-jobs 30  --affix "v7" \
   ...
   --ivector-scale 0.75  --tune-hyper true dev_aspire data/lang $dir
fi
exit 0;
